    ...
    return (
        <div className={style.first_block}>
            ...
            <div className={show ? style.module_show : style.module_hide}>
                <button className={style.close_button} onClick={() => hidden(false)}>&times;</button>
                ...
                <button className={style.button} onClick={(e) => showOffModalForm(e)}>Войти</button>
                <button className={`${style.button} ${style.secondary_button}`} onClick={(e) => showOffModalForm(e)} >Зарегистрироваться</button>
            </div>
        </div>
    );
};
